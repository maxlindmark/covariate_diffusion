...
for(i in c("0", "2.5", "5")){
  ...
  stuff = st_sf( sf_grid,
                 ...
                 "proj2"=as.numeric(A_gs%*%solve(invD,vec2)))   # , log(as.numeric(vec2b)))

  plot( stuff, cex=2, pch=19, border=NA )

  stuff_list[[i]] <- stuff |> mutate(kappa = i)

  sum_list[[i]] <- tibble(kappa = i,
...
stuff2 <- stuff |>
  ...
  mutate(value_sc = scale_values(value),
         .by = c(name, kappa))
...
t <- p +
  geom_sf(data = stuff2 |> filter(type == "point"),
          aes(fill = value_sc, color = value_sc)) +
  geom_text(data = sums2 |> filter(type == "point"),
...
b <- p +
  geom_sf(data = stuff2 |> filter(type == "gmrf"),
          aes(fill = value_sc, color = value_sc)) +
  geom_text(data = sums2 |> filter(type == "gmrf"),
            aes(x = -Inf, y = Inf, label = sum_proj),
            hjust = -0.3, vjust = 2.5, color = "white", size = 2, fontface = "plain") +
