# Code to evaluate mesh sensitivity! Run this first
...
# plot(sf_grid)
mesh <- fmesher::fm_mesh_2d(st_coordinates(sf_loc)[, 1:2], cutoff = 0.09)
mesh$n
...
Date <- Sys.Date()
date_dir <- file.path(root_dir, "results", paste0(Date, "_", f_depth, "_", distribution, "_", "low_cutoff"))
...
# Save optimized parameters to use as starting values in the other runs
par_opt_list <- list()
...
for (cI in seq_along(species_set)) {
  ...
  # Save pars
  param_names <- names(Opt$par)
  param_names[param_names == "beta_j"] <- c("beta_j_1", "beta_j_2")

  opt_pars <- tibble(name = param_names) |>
    mutate(value = Opt$par,
           species = species,
           model = "diffusion") |>
    pivot_wider(names_from = name, values_from = value)

  param_names2 <- names(Opt2$par)
  param_names2[param_names2 == "beta_j"] <- c("beta_j_1", "beta_j_2")

  opt2_pars <- tibble(name = param_names2) |>
    mutate(value = Opt2$par,
           species = species,
           model = "null") |>
    pivot_wider(names_from = name, values_from = value)

  par_opt_list[[cI]] <- bind_rows(opt_pars, opt2_pars)
  ...
  write.csv(Results_cz, file = file.path(date_dir, "low_mesh_Results_cz.csv"))
}

par_opt <- bind_rows(par_opt_list)

write.csv(par_opt, file = file.path(date_dir, "optimized_pars.csv"))
