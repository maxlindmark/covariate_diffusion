...
# plot(sf_grid)
mesh <- fmesher::fm_mesh_2d(st_coordinates(sf_loc)[, 1:2], cutoff = 0.11)
mesh$n
...
Date <- Sys.Date()
date_dir <- file.path(root_dir, "results", paste0(Date, "_", f_depth, "_", distribution, "_", "high_cutoff"))
...
for (cI in seq_along(species_set)) {
  ...
  write.csv(Results_cz, file = file.path(date_dir, "high_mesh_Results_cz.csv"))
