...
# Read data
bering <- read.csv(paste0(here::here(), "/results/2024-08-07_identity_LNP/Results_cz.csv")) |>
...
# Breeding bird case
bb <- read.csv(paste0(here::here(), "/results/2024-08-07_LNP/Results_bb_cz.csv")) |>
  mutate("Diffusion\nfavoured" = ifelse(deltaAIC < 0, "N", "Y"),
         abs_delta_aic = ifelse(abs(deltaAIC) > 2, "<2", ">2")) |>
...
p2 <- mp_bb_s +
  ...
  labs(title = "(b)\nDiffused log(population density)",
       fill = NULL) +
...
ggsave(paste0(here::here(), "/results/figures/bb_maps.pdf"), width = 19, height = 14, unit = "cm")
...
ggsave(paste0(here::here(), "/results/figures/ebs_maps.pdf"), width = 19, height = 14, unit = "cm")
