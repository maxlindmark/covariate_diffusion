...
Type objective_function<Type>::operator() ()
{
  ...
  if(method == "diffusion"){
    //PARAMETER( ln_tau2 );
    PARAMETER( ln_kappa2 );
    //Type prop_const = 1 / (1.0 + exp(2*ln_kappa2)) / exp(ln_tau2);
    //Type ln_tau2 = log(1 / sqrt(4 * M_PI * exp(2*ln_kappa2)));
    // (tau * (kappa^2 M0 + M1))^{-1} M0^{1/2} x
    Eigen::SparseLU< Eigen::SparseMatrix<Type>, Eigen::COLAMDOrdering<int> > lu;
    //Eigen::SparseMatrix<Type> invKd = exp(ln_tau2) * invsqrtM0 * (exp(2*ln_kappa2)*M0 + M1);      //
    //lu.compute(invKd);
    //REPORT( invKd );
    //Eigen::SparseMatrix<Type> invD = exp(ln_tau2) * invM0 * (M0 + exp(2*ln_kappa2)*M0 + M1);
    Eigen::SparseMatrix<Type> invD = invM0 * (M0 + exp(2*ln_kappa2)*M0 + M1) / (1.0 + exp(2*ln_kappa2));      //
    //Eigen::SparseMatrix<Type> invD = invM0 * (M0 + exp(2*ln_kappa2)*M0 + M1) * (1.0 + exp(2.0*ln_kappa2));
    lu.compute(invD);
    REPORT( invD );
